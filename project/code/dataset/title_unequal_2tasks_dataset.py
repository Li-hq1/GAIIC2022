...
class FuseReplaceDatasetWordmatch(Dataset):
    def __init__(self, input_filename, relation_dict_file, vocab_dict, is_train):
        self.is_train = is_train
        with open(relation_dict_file, 'r') as f:
            relation_dict = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        label = 1
        # 随机筛选出总长度中的一个任意长度
        rep_num = random.sample([i for i in range(len(split))], 1)[0]
        if random.random() < 1: # 全部做替换
            label = 0
            rep_idx = random.sample([i for i in range(len(split))], rep_num)
            for i in rep_idx:
                word = split[i]
...
class FuseReplaceDatasetKeyattr(Dataset):
    def __init__(self, input_filename, relation_dict_file, vocab_dict, is_train):
        self.is_train = is_train
        with open(relation_dict_file, 'r') as f:
            relation_dict = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            label = 1
            rep_num = random.sample([1,2], 1)[0]
            if len(key_attr) < rep_num:
                rep_num = len(key_attr)

            if random.random() < 0.6 and rep_num != 0: # 替换
                label = 0
                attr_rep = random.sample(list(key_attr.values()), rep_num)
...
class FuseReplaceDatasetRandommask(Dataset):
    def __init__(self, input_filename, relation_dict_file, vocab_dict, is_train):
        self.is_train = is_train
        with open(relation_dict_file, 'r') as f:
            relation_dict = json.load(f)
